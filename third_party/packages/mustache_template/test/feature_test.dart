...
void main() {
  group('Basic', () {
    test('Variable', () {
      final String output =
          parse('_{{var}}_').renderString(<String, String>{'var': 'bob'});
  ...
  group('Section', () {
    test('Map', () {
      final String output =
          parse('{{#section}}_{{var}}_{{/section}}').renderString(
    ...
    test('Invalid value', () {
      final Exception? ex =
          renderFail('{{#section}}_{{var}}_{{/section}}', <String, int>{
        'section': 42,
      });
    ...
    test('Whitespace in section tags', () {
      expect(
        parse('{{#foo.bar}}oi{{/foo.bar}}')
            .renderString(<String, Map<String, bool>>{
    ...
    test('Odd whitespace in tags', () {
      ...
      render('{{{ #foo }}} {{{ /foo }}}', <String, int>{'#foo': 1, '/foo': 2},
          '1 2');
  ...
  group('Inverse Section', () {
    ...
    test('Invalid value', () {
      ...
      expect((ex! as TemplateException).message,
  ...
  group('Html escape', () {
    test('Escape at start', () {
      final String output =
          parse('_{{var}}_').renderString(<String, String>{'var': '&.'});
  ...
  group('Escape tags', () {
    test('{{{ ... }}}', () {
      final String output =
          parse('{{{blah}}}').renderString(<String, String>{'blah': '&'});
      expect(output, equals('&'));
    });
    test('{{& ... }}', () {
      final String output =
  ...
  group('Lambdas', () {
    void lambdaTest(
            {required String template, dynamic lambda, dynamic output}) =>
        expect(
  ...
  group('Lambda context', () {
    ...
    test('LambdaContext renderSource with value', () {
      ...
      final values = <String, String Function(LambdaContext ctx)>{
        'markdown': (LambdaContext ctx) {
          return ctx.renderSource(ctx.source,
    ...
    test('LambdaContext renderString on non-section throws', () {
      ...
      expect(
        () => t.renderString(
            <String, Object>{'foo': (LambdaContext lc) => lc.renderString()}),
        throwsA(isA<TemplateException>()),
      );
    });
...
void expectFail(Exception? ex, int? line, int? column,

...
void main() {
  ...
  group('Parser', () {
    ...
    test('parse section', () {
      ...
      expectNodes(
          (nodes[1] as SectionNode).children, <Node>[TextNode('def', 11, 14)]);
    });
    ...
    test('parse section standalone tag whitespace on first line', () {
      ...
      final List<Node> nodes = parser.parse();
      expectNodes(nodes,
          <Node>[SectionNode('foo', 2, 10, '{{ }}'), TextNode('ghi', 26, 29)]);
    ...
    test('parse section standalone tag whitespace on last line', () {
      ...
      expectNodes(
          (nodes[0] as SectionNode).children, <Node>[TextNode('def\n', 8, 12)]);
    });
    ...
    test('parse change delimiters', () {
      ...
      expectNodes(
          (nodes[1] as SectionNode).children, <Node>[TextNode('-', 21, 22)]);
    });
